...
execute if block ~ ~-1 ~ #tryashtar.dispense_everything:groups/bottom_only run tag @s add try_dis_ok
...
execute if block ~ ~-1 ~ snow unless block ~ ~-1 ~ snow[layers=1] unless block ~ ~-1 ~ snow[layers=8] run tag @s add try_dis_ok
...
execute if block ~ ~-1 ~ chorus_plant[down=true,up=false] run tag @s add try_dis_ok

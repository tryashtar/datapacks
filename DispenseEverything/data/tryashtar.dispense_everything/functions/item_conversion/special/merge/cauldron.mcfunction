...
execute unless block ~ ~ ~ cauldron[level=0] if data storage tryashtar:dispense_everything item{id:"minecraft:glass_bottle"} run function tryashtar.dispense_everything:item_conversion/special/merge/cauldron/glass_bottle
execute unless block ~ ~ ~ cauldron[level=3] unless entity @s[tag=try_dis_ok] if data storage tryashtar:dispense_everything item{id:"minecraft:potion"}.tag{Potion:"minecraft:water"} run function tryashtar.dispense_everything:item_conversion/special/merge/cauldron/water_bottle
